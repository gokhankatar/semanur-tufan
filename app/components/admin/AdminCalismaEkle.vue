<template>
  <div class="admin-section">
    <h2 v-if="!embedded" class="text-headline-small mb-4">{{ editWork ? 'Çalışma Düzenle' : 'Çalışma Ekle' }}</h2>
    ...
    <v-card variant="outlined" class="pa-4 pa-md-6 position-relative">
      ...
      >
        <div class="d-flex flex-column align-center ga-3">
          ...
          <span class="text-body-medium font-weight-medium">Çalışma kaydediliyor...</span>
        </div>
      </v-overlay>
      ...
      <v-form ref="formRef" @submit.prevent="handleSubmit">
        <div class="cover-upload mb-4">
          ...
          />
          <v-card
            v-if="!coverFile?.length && !editWork?.cover_img"
            variant="outlined"
          ...
          >
            <div class="cover-upload__placeholder-inner">
              ...
              <span class="text-body-medium">Çalışma görseli seçin</span>
          ...
          <v-card v-else variant="outlined" class="cover-upload__preview" rounded="lg">
            <div class="cover-upload__preview-inner">
              <img :src="coverPreviewUrl || (editWork?.cover_img ?? '')" alt="Görsel önizleme" class="cover-upload__img" />
              <div class="cover-upload__preview-info">
                <span class="text-body-small font-weight-medium">{{ coverFile[0]?.name || 'Mevcut görsel' }}</span>
        ...
        <div class="d-flex ga-2">
          <v-btn v-if="editWork" variant="text" @click="emit('cancel')">İptal</v-btn>
          <v-btn type="submit" color="primary" :prepend-icon="editWork ? 'mdi-content-save' : 'mdi-plus'" :loading="loading">
            {{ editWork ? 'Kaydet' : 'Çalışma Ekle' }}
          </v-btn>
        </div>
      </v-form>
    </v-card>
...
const props = defineProps<{ editWork?: Work; embedded?: boolean }>();
const emit = defineEmits<{ saved: []; cancel: [] }>();
const display = useDisplay();
const { addWork, updateWork } = useWorks();
const { uploadFile } = useProfileUpload();

const form = reactive<Omit<Work, "id">>({
  cover_img: props.editWork?.cover_img ?? "",
  work_title: props.editWork?.work_title ?? "",
  work_content: props.editWork?.work_content ?? "",
  work_keywords: props.editWork?.work_keywords ?? [],
});

const coverFile = ref<File[]>(props.editWork?.cover_img ? [] : []);
const coverInputRef = ref<HTMLInputElement | null>(null);
const coverPreviewUrl = ref("");
const coverError = ref("");
const coverDragOver = ref(false);
const keywordsArray = ref<string[]>(props.editWork?.work_keywords ?? []);
const keywordSuggestions = ["resim", "heykel", "fotoğraf", "seramik", "desen", "kolaj"];
...
const handleSubmit = async () => {
  ...
  const hasCover = cover || props.editWork?.cover_img;
  if (!hasCover) {
    error.value = "Görsel gerekli";
    return;
  }
  ...
  try {
    let coverUrl = props.editWork?.cover_img ?? "";
    if (cover) {
      coverUrl = await uploadFile("works", cover);
    }
